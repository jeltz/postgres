...
pg_tde_file_header_read_copy(const char *tde_filename, FILE *file, TDEFileHeader *fheader)
{
	...
	bytes_read = fread(fheader, sizeof(TDEFileHeader), 1, file);
...
pg_tde_read_one_map_entry_copy(FILE *map_file, TDEMapEntry *map_entry)
{
	...
	entries_read = fread(map_entry, sizeof(TDEMapEntry), 1, map_file);
...
pg_tde_replace_key_map_entry(const RelFileLocator *rlocator, const InternalKey *rel_key_data, const TDEPrincipalKey *principal_key)
{
	...
	while (1)
	{
		...
		{
			write_pos = ftell(map_file) - sizeof(TDEMapEntry);
			break;
		}
