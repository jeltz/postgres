...
(
    ...
    CONSTRAINT t6_c29_c30_fkey FOREIGN KEY (c29, c30) REFERENCES t5 (c9, c10)
);
...
--
-- Test foreign key joins where the referenced columns are unique due to matching GROUP BY
--
WITH q AS
(
    SELECT
    ...
    FROM t5
    JOIN t6 KEY (c13, c14) -> t5 (c9, c10)
    GROUP BY t5.c9, t5.c10
)
SELECT
    t7.c15,
    t7.c16
FROM t7
JOIN q KEY (c9, c10) <- t7 (c15, c16);

--
-- Test swapping group by columns columns.
--
WITH q AS
(
    SELECT
...
--
-- The query below should fail, which it does, since it is filtering the referenced side.
--
WITH q AS
(
    SELECT
...
--
-- The query below should fail, since the referencing columns are nullable,
-- which can cause filtering of the referenced side.
--
WITH q AS
(
    SELECT
