...
-- FILTER7: matched filters use exact key identity, not operator families
...
-- rejected, reason: smallint value type is not the exact bigint key type
SELECT *
...
-- accepted, reason: both filters are exact canonical tenant_id = 1::bigint
SELECT *
...
-- rejected, reason: integer value type is not the exact bigint key type
SELECT *
...
-- FILTER8: matched filters require exact key value typmods
...
-- rejected, reason: filter value typmod is not the exact key typmod
SELECT *
FROM filter_typmod_parent_short p
JOIN filter_typmod_child_short c FOR KEY (amount) -> p (amount);

-- accepted, reason: filter value typmods match the key typmod
SELECT *
...
-- FILTER9: matched filters require exact key value collations
...
-- rejected, reason: filter value collation is not the exact key collation
SELECT *
FROM filter_collation_parent_posix p
JOIN filter_collation_child_posix c FOR KEY (code) -> p (code);

-- accepted, reason: filter value collations match the key collation
SELECT *
