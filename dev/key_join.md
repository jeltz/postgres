...
their key columns are direct projected columns.  They preserve row
coverage only when the grouped or distinct columns are known to preserve
all relevant input rows, which requires coverage of a not-null unique key
or an existing row-coverage key.
